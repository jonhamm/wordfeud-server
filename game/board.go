...
func NewBoard(game *_Game) *Board {
	board := Board{
		game:    game,
		squares: [][]Square{},
	}
	board.squares = make([][]Square, game.Dimensions().Height)
	for i := range board.squares {
		board.squares[i] = make([]Square, game.Dimensions().Width)
	}
...
func (board *Board) fillRandomSpecialFields() {
	...
	for _, f := range specialFields {
		for i := 0; i < f.count; i++ {
			n := board.game._rand.Intn(len(normalSquares))
