...
func initialGameState(game *_Game) (*GameState, error) {
	options := game.options
	corpus := game.corpus
	state := &GameState{game: game, fromState: nil, move: nil, tileBoard: make(TileBoard, game.dimensions.Height)}
	...
	for r := Coordinate(0); r < game.dimensions.Height; r++ {
		state.tileBoard[r] = make([]BoardTile, game.dimensions.Width)
		for c := Coordinate(0); c < game.dimensions.Width; c++ {
...
func (state *GameState) AdjacentTile(pos Position, d Direction) (BoardTile, Position) {
	...
	return NullBoardTile, Position{state.game.dimensions.Height + 1, state.game.dimensions.Width + 1}
}
...
func (state *GameState) RelativePosition(pos Position, dir Direction, n Coordinate) (bool, Position) {
	...
	case SOUTH:
		if pos.row+n < state.game.dimensions.Height {
			return true, Position{pos.row + n, pos.column}
	...
	case EAST:
		if pos.column+n < state.game.dimensions.Width {
			return true, Position{pos.row, pos.column + n}

		}
	}
	return false, Position{state.game.dimensions.Height + 1, state.game.dimensions.Width + 1}
}
